...
import { getFirestore,getDocs, collection, onSnapshot, where, query} from 'https://www.gstatic.com/firebasejs/9.22.0/firebase-firestore.js'
...
auth.onAuthStateChanged(async function (user) {
    if (user) 
    {
        ...
        const Iquery = query(collection(db, "PlayerProfileData"), where("userid", "==", userId));
    ...
    if (!querySnapshot.empty) {
      ...
      aboutParagraph.textContent = documentData.About;
        ...
        const Cquery = query(collection(db, "ExperienceData"), where("userid", "==", userId));
        ...
        const aquery= query(collection(db, "PlayersAchievements"), where("userid", "==", userId));
